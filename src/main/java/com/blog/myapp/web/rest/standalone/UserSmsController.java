package com.blog.myapp.web.rest.standalone;

import com.blog.myapp.domain.User;
import com.blog.myapp.security.SecurityUtils;
import com.blog.myapp.security.UserNotActivatedException;
import com.blog.myapp.security.jwt.JWTFilter;
import com.blog.myapp.security.jwt.TokenProvider;
...
import com.blog.myapp.sms.builder.SmsBuilder;
import com.blog.myapp.sms.model.SmsCode;
import com.blog.myapp.sms.model.SmsData;
import com.blog.myapp.web.rest.vm.LoginMobileVM;
...
public class UserSmsController {
    ...
    public ResponseEntity<JWTToken> authorizeByMobile(@Valid @RequestBody LoginMobileVM loginMobileVM) {
        ...
        // Result<Void> result = daYuSmsService.verify(loginMobileVM.getMobile(), loginMobileVM.getCode(), loginMobileVM.getImageCode(), "blog");
        if (result) {
            // 通过mobile查找用户信息。
    ...
    public ResponseEntity<Result<String>> getSmsCodeByCurrentUser(String mobile, @RequestParam(name = "imageCode", required = false) String imageCode) {
        ...
        if (SecurityUtils.getCurrentUserLogin().isPresent()) {
            ...
            if (!existsMobile) {
                // 不存在，发送验证码
                Result<String> result = daYuSmsService.sendVerifyCode(mobile, "blog");
